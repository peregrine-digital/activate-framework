...
/**
 * Screenshot Viewer MCP Server
 * 
 * Provides GitHub Copilot with tools to automatically access and analyze
 * screenshots from any directory (e.g., test failures, generated images).
 * 
 * Tools provided:
 * - list_screenshots: List all available screenshots from a directory
 * - get_screenshot: Get base64-encoded screenshot with metadata
 * - get_recent_screenshots: Get screenshots from a recent time window
 * - search_screenshots: Search for screenshots by name or path
 * 
...
const DEFAULT_SCREENSHOTS_DIR = path.join(WORKSPACE_ROOT, 'cypress/screenshots');
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB limit for base64 encoding

/**
 * Get the screenshots directory, allowing override via parameter
 */
function getScreenshotsDir(directory) {
  if (directory) {
    // If absolute path, use directly; otherwise resolve relative to workspace
 ...
 */
function findScreenshots(screenshotsDir, fileList = [], baseDir = null) {
  // Track the original base directory for relative path calculation
  const screenshotsBase = baseDir || screenshotsDir;
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      findScreenshots(filePath, fileList, screenshotsBase);
    } else if (file.match(/\.(png|jpg|jpeg|gif|webp)$/i)) {
      const relativePath = path.relative(screenshotsBase, filePath);
 ...
 */
function listScreenshots(args = {}) {
  const screenshotsDir = getScreenshotsDir(args.directory);
...
function getScreenshot(args) {
  ...
  const screenshotsDir = getScreenshotsDir(args.directory);
  const fullPath = path.join(screenshotsDir, args.path);
...
/**
 * Get recent screenshots from a time window
 */
function getRecentScreenshots(args = {}) {
  const screenshotsDir = getScreenshotsDir(args.directory);
  const screenshots = findScreenshots(screenshotsDir);
  ...
  if (screenshots.length === 0) {
    return {
      content: [
        {
          type: 'text',
          text: `No screenshots found in ${screenshotsDir}. Ensure the directory exists and contains image files.`,
        },
      ],
    };
  }
  ...
  const content = [
    {
      type: 'text',
      text: `## Recent Screenshots (last ${timeWindowMinutes} minutes)\n\n` +
            `Directory: ${screenshotsDir}\n` +
            `Found ${recentScreenshots.length} screenshot(s) across ${Object.keys(bySpec).length} folder(s).\n\n` +
...
class SimpleMCPServer {
      ...
      if (request.method === 'tools/call') {
        ...
        switch (name) {
          case 'list_screenshots':
            result = listScreenshots(args || {});
            break;
          case 'get_screenshot':
            result = getScreenshot(args || {});
            break;
          case 'get_recent_screenshots':
            result = getRecentScreenshots(args || {});
            break;
  ...
  start() {
    console.error('Screenshot Viewer MCP Server starting...');
    console.error(`Default directory: ${DEFAULT_SCREENSHOTS_DIR}`);
    console.error('Tip: Use the "directory" parameter to specify a different directory');
...
const server = new SimpleMCPServer({
  name: 'screenshot-viewer-mcp-server',
  version: '2.0.0',
});
...
server.registerTools([
  {
    ...
    description: 
      'List all available screenshots from a directory. ' +
      'Useful for getting an overview of images and finding specific screenshots. ' +
      'Returns metadata including names, paths, and modification times.',
    inputSchema: {
      type: 'object',
      properties: {
        directory: {
          type: 'string',
          description: 'Absolute or relative path to the screenshots directory. Defaults to cypress/screenshots if not specified.',
        },
        specFile: {
          type: 'string',
          description: 'Filter by folder/spec file name (partial match, case-insensitive)',
        },
        testName: {
          type: 'string',
          description: 'Filter by test/image name (partial match, case-insensitive)',
        },
        limit: {
          type: 'number',
  ...
  },
  {
    ...
    description: 
      ...
      'Use this after list_screenshots to view the actual screenshot. ' +
      'Path should be the relative path from the list_screenshots response.',
    inputSchema: {
      type: 'object',
      properties: {
        path: {
          type: 'string',
          description: 'Relative path to the screenshot (from list_screenshots)',
        },
        directory: {
          type: 'string',
          description: 'Absolute or relative path to the screenshots directory. Defaults to cypress/screenshots if not specified.',
  ...
  },
  {
    ...
    description: 
      'Get the most recent screenshots by finding and displaying ' +
      'images from a recent time window. ' +
  ...
  },
  {
    ...
    description: 
      'Search for screenshots by name, folder, or path. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        query: {
          type: 'string',
          description: 'Search query (matches name, filename, or path)',
        },
        specFile: {
          type: 'string',
          description: 'Filter by folder/spec file name',
        },
      },
    },
  },

...
The framework has four delivery surfaces: a **compiled Go CLI** with an interactive TUI, a **VS Code extension** with a sidebar control panel, a **native desktop app** (Wails v2), and a **JSON-RPC daemon** that bridges them. All four share the same service layer, preset system, and config schema.
...
The desktop app spawns the same CLI daemon used by the extension and provides a native UI for managing presets, files, and settings across workspaces.

...
func RunFileBrowser(svc commands.ActivateAPI) error {
...
func newFileBrowserModel(svc commands.ActivateAPI) fileBrowserModel {
	state := svc.GetState()
	vals := &fileBrowserValues{}
...
func (m fileBrowserModel) View() string {
	...
	switch m.mode {
	case "browse":
		sections = append(sections, style.DimStyle.Render("  Manage Files"))
		sections = append(sections, style.DimStyle.Render(subtitle))
		sections = append(sections, "")
	...
	case "actions":
		...
		if fs != nil {
			sections = append(sections, style.DimStyle.Render("  "+fs.DisplayName))
			sections = append(sections, style.DimStyle.Render("  "+fileStatusLine(*fs)))
		}
		sections = append(sections, "")
		sections = append(sections, m.form.View())
		sections = append(sections, "")
		sections = append(sections, style.DimStyle.Render("  ↑/↓ navigate · enter select · esc back · ctrl+c quit"))
	case "text":
		sections = append(sections, style.DimStyle.Render("  "+m.textTitle))
		sections = append(sections, "")
...
func (m fileBrowserModel) findSelectedFile() *model.FileStatus {
	for i := range m.files {
...
func buildFileBrowseForm(files []model.FileStatus, selected *string) *huh.Form {
	...
	options = append(options, huh.NewOption(style.DimStyle.Render("← Back to menu"), "_back"))
...
func buildFileActionsForm(fs model.FileStatus, action *string) *huh.Form {
...
func fileActionsForStatus(fs model.FileStatus) []huh.Option[string] {
...
func groupFilesByCategory(files []model.FileStatus) []fileGroup {
	order := make([]string, 0)
	byCategory := make(map[string][]model.FileStatus)
...
func formatFileOption(fs model.FileStatus) string {
	...
	if version != "" {
		parts = append(parts, "  ", style.DimStyle.Render(version))
	}
	if fs.Category != "" {
		parts = append(parts, "  ", style.DimStyle.Render("["+fs.Category+"]"))
	}
	return strings.Join(parts, "")
}

func fileStatusIcon(fs model.FileStatus) string {
	if fs.Override == "excluded" {
		return "🚫"
	}
...
func fileVersionLabel(fs model.FileStatus) string {
	if fs.Override == "excluded" {
		return "excluded"
	}
	if !fs.Installed {
...
func fileStatusLine(fs model.FileStatus) string {
	parts := []string{}
	if fs.Installed {

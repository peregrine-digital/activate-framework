...
func TestIsTelemetryEnabled(t *testing.T) {
	...
	if IsTelemetryEnabled(model.Config{}) {
		t.Fatal("expected disabled by default")
	}

	// Explicitly false
	f := false
	if IsTelemetryEnabled(model.Config{TelemetryEnabled: &f}) {
		t.Fatal("expected disabled when false")
	}

	// Explicitly true
	tr := true
	if !IsTelemetryEnabled(model.Config{TelemetryEnabled: &tr}) {
...
func TestReadTelemetryLogMissing(t *testing.T) {
	old := storage.ActivateBaseDir
	storage.ActivateBaseDir = t.TempDir()
	t.Cleanup(func() { storage.ActivateBaseDir = old })
	entries, err := ReadTelemetryLog()
	if err != nil {
		t.Fatal(err)
...
func TestTelemetryEnabledInConfig(t *testing.T) {
	...
	tr := true
	if err := storage.WriteGlobalConfig(&model.Config{TelemetryEnabled: &tr}); err != nil {
		t.Fatal(err)
	}

	cfg, err := storage.ReadGlobalConfig()
	if err != nil || cfg == nil {

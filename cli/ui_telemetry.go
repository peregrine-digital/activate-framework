...
//   - "menu"  → action menu (Run Now, Toggle, View Log, Back)
//   - "text"  → read-only display (log output, run result)
//
// Called from RunInteractiveMenu as a sub-screen.

// telemetryValues holds heap-allocated form bindings.
...
func (m telemetryModel) View() string {
	...
	if enabled {
		...
		sections = append(sections, dimStyle.Render("  status: disabled"))
	}

	// Show summary if we have log data and are in menu mode
	if m.mode == "menu" {
...
func formatTelemetrySummary(e TelemetryEntry) string {
	var lines []string
...
func formatTelemetryEntry(e TelemetryEntry) string {
	var lines []string
